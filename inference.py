...
def inference_test(model_path, dataset_path, sample_num):
...
def main():
    parser = argparse.ArgumentParser(description='model inference test on PascalVOC dataset')
    parser.add_argument('--model_path', help='model file to predict', type=str, required=True)
    parser.add_argument('--dataset_path',help='path to PascalVOC test dataset', type=str, required=True)
    parser.add_argument('--sample_num', type=int, help='sample number to test in dataset, default=10', default=10)

    args = parser.parse_args()

    inference_test(args.model_path, args.dataset_path, args.sample_num)
